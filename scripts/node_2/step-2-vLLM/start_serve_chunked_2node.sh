#!/bin/bash
# =============================================================================
# Step 2 (2-Node, 8 GPUs): Start 8 vLLM servers with CHUNKED PREFILL
# Node 1 (from .node_info):   GPUs 0-3, ports 8000-8003
# Node 2 (from .node_info_2): GPUs 0-3, ports 8004-8007
#
# Key difference from start_serve_multi_2.sh:
#   --enable-chunked-prefill: splits long prefills into chunks so decode
#   requests aren't blocked. Improves throughput for mixed long/short loads.
#
# Usage: bash scripts/step-2-vLLM/start_serve_chunked_2node.sh [--fp8]
...
echo -e "${CYAN}=========================================="
echo "  Starting 8x vLLM: Qwen3.5-27B (CHUNKED PREFILL)"
...
echo "  Chunked Prefill: ENABLED (chunk=8192 tokens)"
...
# Build vLLM args — with chunked prefill
COMMON_ARGS=(
    --model "$MODEL"
    --language-model-only
...
echo -e "${GREEN}=========================================="
echo "  ${READY_COUNT}x vLLM Servers Ready (Chunked Prefill)"
echo ""
echo "  Node 1 ($NODE_1):"
for gpu_id in $(seq 0 $((NUM_GPUS_PER_NODE-1))); do
    port=$((BASE_PORT_NODE1 + gpu_id))
...
echo ""
echo "  Chunked Prefill: ON (chunk=8192)"
