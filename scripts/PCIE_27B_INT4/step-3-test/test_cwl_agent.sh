#!/bin/bash
# =============================================================================
# Test the PCIE A100 vLLM server (Qwen3.5-27B-AWQ-4bit) with cwl_agent_test.py
# Lightweight agentic test: model writes CWL workflow, runs cwltool, debugs
#
# All artifacts go to: tests/<timestamp>_cwl_agent/
# Usage: bash scripts/PCIE_27B_INT4/step-3-test/test_cwl_agent.sh
# =============================================================================
set -euo pipefail
...
OUTPUT_DIR="$TESTS_DIR/${TIMESTAMP}_cwl_agent"
mkdir -p "$OUTPUT_DIR"

echo -e "${CYAN}=========================================="
echo "  CWL Agent Test — Qwen3.5-27B-AWQ-4bit"
echo "  vLLM:   $API_URL"
...
if ! curl -s --connect-timeout 5 "${API_URL}/models" > /dev/null; then
    ...
    echo "Start it first: bash scripts/PCIE_27B_INT4/serve.sh"
    exit 1
fi
...
# Locate cwltool from local python
...
python3 "$SCRIPT_DIR/test_cwl_agent.py" \
    --api-url "$API_URL" \
    --model "$MODEL_NAME" \
    --cwltool "$CWLTOOL_BIN" \
